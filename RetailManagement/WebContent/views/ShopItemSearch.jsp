...
<script type="text/javascript">
	function fnItemName(itemId, itemName,retailerMRP) {
...
<body>
	...
	<form name="itemSearch" method="post">
		...
		<table align="Center" border="2" cellpadding="2" class="table table-hover">
			...
			<tbody>
				<tr class="success">
					<td><a href="javascript:fnItemName('<%=rs.getString("ITEMS_MASTER_ID")%>','<%=rs.getString("ITEMS_MASTER_NAME")%>','<%=rs.getString("ITEMS_MASTER_RETAILMRP")%>' "><%=rs.getString("ITEMS_MASTER_ID")%></a></td>
					<td><a href="javascript:fnItemName('<%=rs.getString("ITEMS_MASTER_ID")%>','<%=rs.getString("ITEMS_MASTER_NAME")%>','<%=rs.getString("ITEMS_MASTER_RETAILMRP")%>')"><%=rs.getString("ITEMS_MASTER_NAME")%></a></td>
					<td><a href="javascript:fnItemName('<%=rs.getString("ITEMS_MASTER_ID")%>','<%=rs.getString("ITEMS_MASTER_NAME")%>','<%=rs.getString("ITEMS_MASTER_RETAILMRP")%>')"><%=rs.getString("ITEMS_MASTER_RETAILMRP")%></a></td>	
				</tr>
