...
<script type="text/javascript">
	...
	function fnRetailerDel(retailerId, retailerName){
		window.opener.delRetailerId.value=retailerId;
		window.opener.delRetailerId.focus();
		window.opener.delRetailerName.value=retailerName;
		window.opener.delRetailerName.focus();
...
<body style="background-image: url('../img/Supermarket.jpg');  background-repeat: no-repeat;">
	...
	<form name="retailerSearch" method="post">
		...
		<table align="Center" border="2" cellpadding="2" class="table table-hover">
			...
			<tbody>
				<tr class="success">
					<td><a href="javascript:fnRetailerDel('<%=rs.getString("RETAILERS_MASTER_ID")%>','<%=rs.getString("RETAILERS_MASTER_NAME")%>') "><%=rs.getString("RETAILERS_MASTER_ID")%></a></td>
					<td><a href="javascript:fnRetailerDel('<%=rs.getString("RETAILERS_MASTER_ID")%>','<%=rs.getString("RETAILERS_MASTER_NAME")%>')"><%=rs.getString("RETAILERS_MASTER_NAME")%></a></td>
